...
export function setAuthSession({ token, usuario }) {
  ...
  // Compatibilidad con APIs viejas del front.
...
function obtenerPrimerError(data) {
  ...
  if (Array.isArray(data.pendientes) && data.pendientes.length > 0) {
    return `Pendientes: ${data.pendientes.join(", ")}`;
  }
...
export async function http(path, { method = "GET", body, headers } = {}) {
  ...
  // Muy importante:
  // Si el body es FormData, NO pongas Content-Type manualmente.
...
export async function httpBlob(path, { method = "GET", body, headers } = {}) {
  const finalHeaders = {
    ...getAuthHeader(),
    ...(headers || {}),
  };
